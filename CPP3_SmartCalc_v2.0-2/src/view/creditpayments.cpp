...
void CreditPayments::receiveAnnuityData(double s, double p, int n,
                                        QDate startDate) {
  double payment = s21::CalcController::AnnuityPayment(s, p, n);
  double overpayment = 0;
  double totalSum = s;
  QDate date = startDate;
  for (int i = 0; n > 0; i++, n--) {
    date = date.addMonths(1);
    double interestSum = s21::CalcController::InterestSum(s, p);
...
void CreditPayments::receiveDifferentiatedData(double s, double p, int n,
  ...
  for (int i = 0; n > 0; i++, n--) {
    date = date.addMonths(1);
    interestSum = s21::CalcController::InterestSum(s, p);
    overpayment += interestSum;
